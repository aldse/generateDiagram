...
class UserController {
    static async register(req, res) {
        try {
            const { name, dateBirth, email, cpf, edv, password, confirmPassword, cep } = req.body;
    ...
    static async login(req, res) {
        try {
            ...
            const token = jwt.sign({ id: user._id, isAdm: user.isAdm }, process.env.JWT_SECRET, { expiresIn: '24h' });
